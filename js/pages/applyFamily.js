...
import { applyJoinFamily } from '../actions/family';
...
class comp extends Component {
    ...
    componentWillMount() {
        console.log('applyFamily componentWillMount');
    }
    ...
    shouldComponentUpdate(nextProps, nextState) {
        ...
        if (this.props.time != nextProps.time && nextProps.status == TYPES.APPLY_JOIN_FAMILY_ERROR) {
            this.dropdown.alertWithType('error', '操作未成功', nextProps.message);
            return false;
        }
        if (this.props.time != nextProps.time && nextProps.status == TYPES.APPLY_JOIN_FAMILY_OK) {
    ...
    submit() {
        if (!this.state.id || !this.state.message) {
        ...
        this.props.dispatch(applyJoinFamily(opt));
    }
            ...
            <Container style={[commonStyle.wrapper, styles.container]}>
                <Content style={styles.content}>
                    ...
                    <View style={styles.btnWrap}>
                        <Button primary block style={styles.btn} onPress={this.submit}><Text > 提交申请  </Text></Button>
                    </View>
