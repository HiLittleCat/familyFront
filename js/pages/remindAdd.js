...
class Remind extends Component {
    constructor(props) {
        ...
        let user = this.props.members[index];
        ...
        this.state = {
            ...
            toUserId: user._id,
            message: '',
            completeDate: nextDate,
            selectedValue: this.props.user.name
        }
    ...
    shouldComponentUpdate(nextProps, nextState) {
        ...
        if (this.props.time != nextProps.time && nextProps.status == TYPES.CREATE_REMIND_ERROR) {
            this.dropdown.alertWithType('error', '操作未成功', nextProps.message);
            return false;
        }
        if (this.props.time != nextProps.time && nextProps.status == TYPES.CREATE_REMIND_OK) {
    ...
    addRemind() {
        ...
        this.props.dispatch(RemindAction.createRemind(opt));
    }
            ...
            <Content>
                <View style={styles.formatWrap}>
                    ...
                    <ModalDropdown
                        ...
                        onSelect={(index, value) => { this.setState({ selectedValue: value.name }); this._membersSelect(index, value) }}
                    >
                ...
                <TouchableOpacity style={styles.btnWrap} onPress={() => { self.addRemind() }}>
